...
export default function AllUsers(props) {
  ...
  const namesOfColumns = Object.keys(props.data.jobs[0]).filter(
    (name) =>
      name !== 'id' &&
      name !== 'user_id' &&
  ...
  uniqueEntries.forEach((value) => {
    const { name, count, details } = countStats(
      props.data.jobs,
      value,
      filterTerm,
    )
  ...
  return (
    <div>
      <Header handleChange={handleChange} goingTo="allUserPage" />
      <span>
        <p className="fw-lighter text-muted">
          This stats is gotten from all users saved jobs
        </p>
      </span>
