...
export default function User(props) {
      ...
      <>
        ...
        <span>
          <p className="fw-lighter text-muted">
            Oops! You have no saved jobs to show stats for. Please add a job to
            see stats
          </p>
        </span>
      </>
  ...
  const namesOfColumns = Object.keys(props.data[0]).filter(
    (name) =>
      name !== 'id' &&
      name !== 'user_id' &&
  ...
  const countStats = (arr, item, filterTerm) => ({
    name: item,
    count: arr.reduce(
      (count, elem) => (item === elem[filterTerm] ? count + 1 : count),
      0,
    ),
  ...
  uniqueEntries.forEach((value) => {
    const { name, count, details } = countStats(props.data, value, filterTerm)

    // will hold information for all data where unique entry name appears
    const detailsData = []

    namesOfColumns
      .filter((name) => name !== filterTerm)
      .forEach((name) => {
        ...
        innerUniqueEntries.forEach((entry) => {
          ...
          const { name: detailsName, count: detailsCount } = countStats(
            details,
            entry,
            name,
          )
  ...
  return (
    <div>
      <Header handleChange={handleChange} goingTo="userPage" />
      <span>
        <p className="fw-lighter text-muted">
          This stats is gotten from your saved jobs
        </p>
      </span>
      <Table filteredData={filteredData} filterTerm={filterTerm} />
    </div>
  )
}
